...
EduAgent 是面向中国软件杯 A3 赛题的多智能体个性化学习系统。当前公共基线提供自然语言学习画像、个性化学习路径、SQLite 持久化、资源任务编排协议、任务状态和 SSE 进度；五类真实资源 Agent、Reviewer、Evaluation 和真实 LLM 将由后续阶段接入。
...
- `LLM_API_KEY`：当前留空；真实密钥只能写入被忽略的 `.env` 或系统环境变量。
...
- Profile Agent：`development_heuristic`，最终需替换为结构化LLM抽取。
- Planner Agent：`development_rule_based`，最终需替换为知识库与LLM规划。
