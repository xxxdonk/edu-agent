...
当前第一阶段固定公共契约并提供输入相关的开发适配器：Profile Agent 使用启发式抽取，Planner Agent 使用规则规划。两者均通过 `generation_mode`/`extraction_mode` 明确标记，不冒充大模型结果；Day 2 将替换为统一大模型客户端驱动的结构化输出，同时保持 API 数据模型不变。
...
- 最终演示前必须将 `development_heuristic` 和 `development_rule_based` 替换为结构化大模型模式或明确标记的可追溯缓存结果。
