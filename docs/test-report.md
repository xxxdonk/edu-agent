...
集成分支：`agent1/final-regression`（与 `develop@3e759b9` 一致）
...
| 检查 | 命令 | 结果 |
|---|---|---|
| 后端测试 | `.\.venv\Scripts\python.exe -m pytest -q` | 97 passed |
...
本轮按“失败后最多透明重试一次”运行。A、B 完成画像 v1→v2、真实评价和带 `adjustment_reason` 的重规划；C 的第二次尝试在评价后重规划阶段因真实 DeepSeek 结构输出未通过 Pydantic 校验而显式降级，验证器以 `updated_path_not_llm_structured` 失败退出，没有把 fallback 写成成功。三例画像、路径和资源内容保持差异化，资源阶段均未触发 fallback。
...
- 真实模型响应时间和结构遵循度受外部服务影响；本次 B 透明重试后成功，C 在一次透明重试后仍于评价后 Planner 阶段显式 fallback。
...
- 当前处理：未修改后端；前端如实展示后端持久化结果，待 Profile/Evaluation 集成负责人修复 fallback 摘要解析。
