...
本轮根因是 Profile、Planner 和五资源 fallback 存在无条件机器学习默认值，RAG 只加载机器学习资料但缺少课程隔离，前端 placeholder、快捷建议和案例 A/B 也持续暗示机器学习。现已新增仅后端内部使用的 `SubjectContext`，识别教育阶段、开放学科名称、学科族、年级、主题、目标与时间，不进入公共 API。

关键结果：

- “怎么学高中数学”立即识别为高中、数学学科，不再询问大学专业；“我上高中”不会覆盖已确认课程。
- 课程切换会保留通用偏好和时间预算，清理旧课程的薄弱点、目标和历史。
- Planner 按数学、语言、理科、人文、计算机、工程、艺术等学科逻辑生成 3 至 8 步路径。
- 五类资源保持原公共类型；非计算学科的 `coding` 显示并生成“应用实践任务”。
- RAG 查询包含课程、主题和目标；非机器学习课程不会检索机器学习库，零命中时透明使用通用模型或规则。
- Reviewer 检查课程、学段、来源和泄漏词，并对 `subject_mismatch` 最多定向重写一次。
- 前端提供 17 个全科学习示例，刷新与新会话不复用固定学生 ID。

| 检查 | 结果 |
|---|---|
| 20 项学科识别矩阵及闭环专项 | 25 passed |
| 后端全量测试 | 177 passed，1 条既有 Starlette/httpx 弃用警告 |
| 前端全量测试 | 72 passed |
| vue-tsc | 通过 |
| Vite 生产构建 | 通过，3392 个模块转换；保留非阻断的大 chunk 警告 |
| 三尺寸视觉检查 | `1366×768`、`1280×720`、`768×900` 均可用，无文字或控件互相遮挡 |
| 演示预检 | 18 PASS / 2 FAIL；仅缺少 `LLM_MODEL` 与 `LLM_API_KEY` |
| 公共 API / Schema / 数据库 | 无变化 |
| 新增依赖 / 新资源类型 | 均无 |

当前真实模型参数仍未配置，因此本轮不能声称完成真实 LLM 冷链路。已使用三个全新学生 ID 通过真实 HTTP 接口完成 development fallback 烟雾验证，且全程保留模式与来源标记：

| HTTP 烟雾案例 | 画像与路径 | 五类资源与审校 | Quiz 与动态更新 | 串科检查 |
|---|---|---|---|---|
| 高中数学 | 课程正确，6 步路径 | 5/5，全部 approved | 9 题，画像与路径均触发更新 | 无机器学习泄漏 |
| 大学英语 | 课程正确，6 步路径 | 5/5，全部 approved；实践为 Markdown | 9 题，画像与路径均触发更新 | 无机器学习泄漏 |
| 自动控制原理 | 课程正确，6 步路径 | 5/5，全部 approved；实践为 Python | 9 题，画像与路径均触发更新 | 无机器学习泄漏 |

三个案例的非机器学习本地知识库均为零命中，五类资源都按契约携带透明 `general-model` 来源标记。烟雾验证没有重复创建请求；评价各故意错一道题，掌握度均为 `0.9444`，用来验证画像和路径更新链路，而不是宣称满分表现。

持久化复查发现，启发式 Planner 曾把“期末考试”目标片段当成普通知识点。随后已将考试目标改为规划条件，为路径末步预留“错题复盘与模拟练习”，并增加防止时间预算、资源偏好进入步骤标题的回归断言。
